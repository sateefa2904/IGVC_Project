...
ANGLE_TOL_DEG        = 90      # effectively disabled now (kept for reference)

TOP_K_COMPONENTS     = 8

# Where we want the line to be (fraction of image width)
# 0.2 = left side, 0.5 = center, 0.8 = right side
TARGET_X_RATIO = 0.43   # keep line on left side of screen
...
def process_frame(frame_bgr, tracker: SimpleLineTracker):
    ...
    for cnt in contours:
        ...
        if aspect < MIN_ASPECT_H_OVER_W:
            continue
        # angle filter removed / relaxed; we rely mostly on area + aspect
    ...
    return out, mask_debug, forward, turn, (angle_deg if angle_deg is not None else 0.0)
...
def run_webcam():
    ...
    while True:
        ...
        result, mask_debug, forward, turn, angle_deg = process_frame(frame, tracker)
