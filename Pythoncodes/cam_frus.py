...
# ---- NEW: lock to a specific camera index ----
# Try 1, 2, 3... until you get the Logitech C920.
CAM_INDEX = 0
CAM_BACKENDS = [cv2.CAP_DSHOW, cv2.CAP_MSMF, cv2.CAP_ANY]
# ----------------------------------------------
...
def open_windows_camera():
    """
    Lock to a single camera index (CAM_INDEX), trying a few backends.
    This avoids accidentally grabbing the laptop's front camera.
    """
    for backend in CAM_BACKENDS:
        print(f"[CAM] Trying index {CAM_INDEX} with backend {backend}...")
        cap = cv2.VideoCapture(CAM_INDEX, backend)
        time.sleep(0.3)
        if not cap.isOpened():
            print("[CAM]   Not opened on this backend.")
            cap.release()
            continue
        ret, frame = cap.read()
        if ret:
            print(f"[CAM] Using camera index {CAM_INDEX} with backend {backend}")
            return cap
        else:
            print("[CAM]   Opened but could not read frame.")
            cap.release()

    print(f"[CAM] Could not open camera at index {CAM_INDEX} on any backend.")
    return None
