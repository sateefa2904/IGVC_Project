...
TARGET_X_RATIO = 0.25   # keep line on left side of screen

# Control gains (normalized error in [-1, 1])
Kp = 0.9
Kd = 0.2
...
def rect_angle_from_vertical(rect):
    """|angle from vertical| in degrees for minAreaRect."""
...
def process_frame(frame_bgr, tracker: SimpleLineTracker):
    ...
    for cnt in contours:
        ...
        if aspect < MIN_ASPECT_H_OVER_W:
            continue
        ang_from_vert = rect_angle_from_vertical(rect)
    ...
    return out, mask_white, forward, turn
...
def run_webcam():
    ...
    while True:
        ...
        result, mask_white, forward, turn = process_frame(frame, tracker)
        ...
        tl_text = f"{'MIRROR ' if MIRROR_FRAME else ''}Single-line left-follow; ESC/q to quit"
        debug_view = stack_debug_views(frame, mask_white, result, turn, tl_text)
