...
PORT = "/dev/ttyACM0"   # <-- Jetson: adjust as needed (e.g., /dev/ttyACM0)
BAUD = 19200
...
# Turn thresholds (steering from lane-follow PD)
...
MIRROR_FRAME = False   # Flip if camera is mirrored
...
SHOW_WINDOW        = False   # False on headless Jetson if you don't want any GUI
SHOW_STACKED_DEBUG = False  # True = 4-panel debug; False = just overlay view
...
TURN_TIME_RIGHT      = 0.6   # seconds turning RIGHT after left lane ends
GO_TIME_AFTER_RIGHT  = 2.5   # seconds going FORWARD after that right turn
TURN_TIME_LEFT       = 0.8   # seconds turning LEFT after right lane ends
GO_TIME_AFTER_LEFT   = 4   # seconds going FORWARD after that left turn
CAMERA_SETTLE_TIME   = 0.0   # seconds waiting after opening a camera
...
def tune_camera_for_speed(cap, label: str):
    # You can tweak these; Jetson often likes 640x480 or 960x540
...
def open_side_camera(side: str):
    ...
    ret, frame = cap.read()
    if not ret:
        print(f"[CAM] {label}: could not read initial frame after tuning")
        cap.release()
        return None, None
...
def run_hardcoded_left_right_left():
    ...
    try:
        ...
        # ===== TRANSITION 1: LEFT -> RIGHT (no camera open) =====
        print("[STATE] >>> Transition 1: LEFT -> RIGHT (no camera, purely timed motion)")
        print(f"[STATE] Turning RIGHT for {TURN_TIME_RIGHT:.2f} s")
        send_cmd(CMD_RIGHT)
        time.sleep(TURN_TIME_RIGHT)

        print(f"[STATE] Going FORWARD for {GO_TIME_AFTER_RIGHT:.2f} s")
        send_cmd(CMD_UP)
        time.sleep(GO_TIME_AFTER_RIGHT)

        print("[STATE] Stopping after RIGHT turn + forward")
        send_cmd(STOP_CMD)

        # Open RIGHT camera now
        print("[STATE] >>> Opening RIGHT camera after transition")
        cap, tracker = open_side_camera("RIGHT")
        if cap is None:
            print("[STATE] Could not open RIGHT camera, exiting.")
            return

        if CAMERA_SETTLE_TIME > 0:
            print(f"[STATE] Camera settle for {CAMERA_SETTLE_TIME:.2f} s")
        ...
        # ===== TRANSITION 2: RIGHT -> LEFT (no camera open) =====
        print("[STATE] >>> Transition 2: RIGHT -> LEFT (no camera, purely timed motion)")
        print(f"[STATE] Turning LEFT for {TURN_TIME_LEFT:.2f} s")
        send_cmd(CMD_LEFT)
        time.sleep(TURN_TIME_LEFT)

        print(f"[STATE] Going FORWARD for {GO_TIME_AFTER_LEFT:.2f} s")
        send_cmd(CMD_UP)
        time.sleep(GO_TIME_AFTER_LEFT)

        print("[STATE] Stopping after LEFT turn + forward")
        send_cmd(STOP_CMD)

        # Final LEFT camera open
        print("[STATE] >>> Re-opening LEFT camera after transition")
        cap, tracker = open_side_camera("LEFT")
        if cap is None:
