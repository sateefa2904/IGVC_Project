...
# Debug toggle
SHOW_STACKED_DEBUG = False   # True = heavy 4-panel debug; False = lighter
...
# Transition timing (you can tweak these)
TURN_TIME_RIGHT      = 3      # seconds turning RIGHT after left lane ends
GO_TIME_AFTER_RIGHT  = 5      # seconds going FORWARD after that right turn
TURN_TIME_LEFT       = 3      # seconds turning LEFT after right lane ends
GO_TIME_AFTER_LEFT   = 5      # seconds going FORWARD after that left turn
CAMERA_SETTLE_TIME   = 0.2      # extra wait after transition before using lane
...
def schedule_forward_after_delay(delay_s: float, send_cmd):
    """
    Soft timer: after delay_s seconds, send Forward Half once.
    """
    def worker():
        time.sleep(delay_s)
        print(f"[TIMER] {delay_s:.2f}s elapsed -> sending {CMD_UP}")
        send_cmd(CMD_UP)
...
def schedule_stop_after_delay(delay_s: float, send_cmd):
    """
    Soft timer: after delay_s seconds, send Stop once.
    """
    def worker():
        time.sleep(delay_s)
        print(f"[TIMER] {delay_s:.2f}s elapsed -> sending {STOP_CMD}")
        send_cmd(STOP_CMD)
    t = threading.Thread(target=worker, daemon=True)
    t.start()
...
# =========================
# Main: LEFT → RIGHT → LEFT with timers + concurrent camera open
# =========================
...
def run_hardcoded_left_right_left():
    ...
    try:
        ...
        # ===== TRANSITION 1: LEFT -> RIGHT (timers + open camera concurrently) =====
        print("[STATE] >>> Transition 1: turning RIGHT then FORWARD (timers) and opening RIGHT camera")
        trans1_total = CAMERA_SETTLE_TIME

        # Start turning right immediately
        send_cmd(CMD_RIGHT)
        # Timers to switch to forward and then stop
        ...
        # Start opening RIGHT camera right away (in this same thread)
        cap, tracker = open_side_camera("RIGHT")
        if cap is None:
            print("[STATE] Could not open RIGHT camera during Transition 1, exiting.")
            return

        # Loop until transition time is over (no lane processing yet)
        t0 = time.time()
        # while True:
        #     now = time.time()
        #     elapsed = now - t0
        #     if elapsed >= trans1_total:
        #         print("[STATE] Transition 1 time window elapsed.")
        #         break

        #     # Just show a small status screen (optional)
        #     blank = np.zeros((240, 320, 3), dtype=np.uint8)
        #     cv2.putText(blank, f"Transition 1 (L->R)", (10, 40),
        #                 cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
        #     cv2.putText(blank, f"t = {elapsed:4.2f}s / {trans1_total:4.2f}s", (10, 80),
        #                 cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)
        #     cv2.imshow("Lane Debug", blank)
        #     key = cv2.waitKey(1) & 0xFF
        #     if key == ord('q') or key == 27:
        #         print("[STATE] Quit requested during Transition 1.")
        #         return

        # Ensure we end transition stopped
        send_cmd(STOP_CMD)
        ...
        # ===== TRANSITION 2: RIGHT -> LEFT (timers + open camera concurrently) =====
        print("[STATE] >>> Transition 2: turning LEFT then FORWARD (timers) and opening LEFT camera")
        trans2_total = CAMERA_SETTLE_TIME
        ...
        cap, tracker = open_side_camera("LEFT")
        if cap is None:
            print("[STATE] Could not reopen LEFT camera during Transition 2, exiting.")
            return
        ...
        #     now = time.time()
        #     elapsed = now - t0
        #     if elapsed >= trans2_total:
        #         print("[STATE] Transition 2 time window elapsed.")
        #         break

        #     blank = np.zeros((240, 320, 3), dtype=np.uint8)
        #     cv2.putText(blank, f"Transition 2 (R->L)", (10, 40),
        #                 cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
        #     cv2.putText(blank, f"t = {elapsed:4.2f}s / {trans2_total:4.2f}s", (10, 80),
        #                 cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)
        #     cv2.imshow("Lane Debug", blank)
        #     key = cv2.waitKey(1) & 0xFF
        #     if key == ord('q') or key == 27:
        #         print("[STATE] Quit requested during Transition 2.")
        #         return
