...
void writeSpi0Data(uint8_t data)
{
    SSI0_DR_R = data;
...
{
    SSI0_DR_R = data;
    while (SSI0_SR_R & SSI_SR_BSY);       // Wait until TX completes
//    while (!(SSI0_SR_R & SSI_SR_RNE));    // Wait for RX data
//    volatile uint8_t dummy = SSI0_DR_R;   // Clear RX FIFO
}
...
uint32_t readSpi0Data()
{
    while (!(SSI0_SR_R & SSI_SR_RNE));  // Wait for data
    return SSI0_DR_R;
}
